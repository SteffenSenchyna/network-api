...
def putS3(hostname, config):
    ...
    s3_object = s3_client.Object("network-conf", f'{hostname}/{formated_date}.txt')
    try:
...
def postBackup(request):
    ...
    for i in ips:
        device = driver(hostname=i, username=os.environ["USER_NAME"], password=os.environ["PASSWORD"], optional_args={'secret': os.environ["SECRET"]})
        try:
            device.open()
